/** Synthetic Auth/data fixtures for a disposable local Supabase stack only. Never sends email. */
...
if (
  ...
  throw new Error(
    "Synthetic seeding is restricted to local Supabase on http://localhost:54321.",
  );
const key = process.env.SUPABASE_SECRET_KEY;
const password = process.env.DEMO_SEED_PASSWORD;
if (!key || !password || password.length < 12)
  throw new Error(
    "Provide the LOCAL service key and a DEMO_SEED_PASSWORD of at least 12 characters.",
  );
...
if (existing.error)
  throw new Error("Apply migrations to the local stack first.");
if (existing.count)
  throw new Error(
    "This script requires an empty disposable database. Reset the LOCAL stack before seeding again.",
  );
const s = seedStore();
...
console.log(
  "Local synthetic data installed. No invitations or email sent. Sign in using DEMO_SEED_PASSWORD and one of:",
);
