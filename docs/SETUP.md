...
The application runs in demo without external services. The owner designated `sorblhbsciedhyihjaum` as the **test backend** on September 16, 2026. All six repository migrations are applied; migration history matches repository timestamps. Both Storage buckets are private. Public Auth signup is disabled; email/password is enabled and email confirmation remains required. Synthetic owner, technician and client accounts are installed. Vercel connection variables, the canonical app origin and the deployed Auth callback are configured for https://app.nettech.ms. Core deployed API checks pass. The send-only Resend sender and Auth SMTP are configured; provider simulator and Auth callback checks pass. The two explicitly approved staff setup emails have been submitted to Resend. Staff acceptance, human inbox delivery and real-client production acceptance remain pending. See `EMAIL-SETUP.md`. Use separate projects and environment scopes for staging and production.
...
The migration `20260916223238_technician_business_routing.sql` adds the dispatch-only default-technician directory and trusted technician invitation setup. Apply it with the other reviewed migrations before using the connected owner/team screen. This migration is applied to the designated hosted test project. The two explicitly approved staff setup emails have been sent; real-client invitations remain pending review.
